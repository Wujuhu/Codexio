...
from aiquota.dashboard import Dashboard, HistoryAssignmentEditor, PriceEditor, RequestDetails, SessionTooltip, TwoLineText
...
def test_open_page_reopens_hidden_main_window(app):
    ...
    assert dashboard._stack.currentIndex() == 1
...
def test_prices_only_show_available_codex_models_and_two_decimals(app):
    dashboard = Dashboard(AppSettings(), {}, {})
...
def test_price_headers_share_alignment_and_numeric_column_space(app, theme):
    ...
    for column in range(6):
        expected = (Qt.AlignmentFlag.AlignLeft if column < 2 else Qt.AlignmentFlag.AlignRight) | Qt.AlignmentFlag.AlignVCenter
    ...
    widths = [view.columnWidth(column) for column in range(2, 6)]
...
def test_latest_request_is_embedded_and_home_amount_is_larger(app):
    from aiquota.dashboard import RequestContent
    dashboard = Dashboard(AppSettings(), {}, {})
    dashboard.open_page("overview")
    data = record()
    dashboard.apply_data({"records": [data], "summaries": {"today": {"tokens": 1100, "usd": 80.01}}})
    app.processEvents()
    assert isinstance(dashboard._latest_panel, RequestContent)
    assert dashboard._latest_panel.session_id.text() == data["session_id"]
    assert not dashboard._latest_panel.isWindow()
    assert dashboard._metrics["today"][1].text() == "$80.01"
    assert dashboard._metrics["today"][1].font().pixelSize() >= 20
    dashboard.apply_data({"records": [record(2, session_id="new-session")]})
    assert dashboard._latest_panel.session_id.text() == "new-session"
    dashboard.deleteLater()


def test_trends_open_to_week_and_model_column_has_no_tier(app):
    dashboard = Dashboard(AppSettings(), {}, {})
    dashboard.open_page("trends")
    assert dashboard._trend_period.currentData() == "week"
    assert dashboard._granularity.currentData() == "day"
    dashboard.apply_data({"records": [record(service_tier="priority")]})
    dashboard.open_page("logs")
    assert dashboard._log_table.horizontalHeaderItem(1).text() == "模型"
    assert dashboard._log_table.item(0, 1).text() == "gpt-6-astra"
    assert dashboard._log_table.columnWidth(1) < 185
    assert dashboard._log_table.horizontalHeaderItem(2).text() == "Fast"
    assert dashboard._log_table.item(0, 2).text() == "开启"
    dashboard.deleteLater()


@pytest.mark.parametrize("tier,expected", [("priority", "开启"), ("fast", "开启"), ("default", "关闭"),
                                           ("standard", "关闭"), (None, "未知"), ("auto", "未知"), ("flex", "未知")])
def test_fast_column_never_infers_missing_tier(tier, expected):
    from aiquota.dashboard import fast_mode_label
...
def test_home_reset_card_times_and_minimal_request(app):
    ...
    dashboard.open_page("overview")
    labels = [label.text() for label in dashboard._latest_panel.findChildren(QLabel)]
    assert "Total Token" in labels
    assert "输入 Token（含缓存）" in labels
    assert "输出 Token" in labels
    assert "Fast 模式" in labels
    overview = dashboard._stack.widget(0).widget().layout()
    assert overview.itemAt(overview.count() - 2).widget() is dashboard._latest_box
    assert overview.itemAt(overview.count() - 1).widget() is dashboard._overview_chart.parentWidget()
    ...
    assert dashboard._reset_count.text() == "3 次"
    assert "今天" in dashboard._quota_resets["week"].text() or "明天" in dashboard._quota_resets["week"].text()
    dashboard.deleteLater()
...
def test_log_short_columns_are_compact_and_preview_icon_stays_visible(app):
    ...
    dashboard.open_page("logs")
    app.processEvents()
    widths = [dashboard._log_table.columnWidth(i) for i in range(10)]
    assert widths[2] < widths[1] and widths[8] < widths[7]
    assert dashboard._log_table.horizontalHeaderItem(8).text() == "状态"
    assert dashboard._log_table.horizontalHeaderItem(9).text() == "来源"
    cell = dashboard._log_table.cellWidget(0, 7)
    icon = cell.findChild(SessionInfoButton)
    assert icon.geometry().right() <= cell.width()
...
def test_trend_filters_only_offered_models_and_preserves_selection(app):
    dashboard = Dashboard(AppSettings(), {}, {})
    dashboard.open_page("trends")
    records = [record(1), record(2, model="gpt-5.6-sol", total_tokens=2200),
               record(3, model="internal-model", total_tokens=3300)]
...
def test_initial_progress_failure_is_visible_and_does_not_spin_forever(app):
    ...
    assert dashboard._latest_panel.text() == "请求记录加载失败"
    dashboard.deleteLater()
