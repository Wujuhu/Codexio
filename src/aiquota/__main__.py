...
if __name__ == "__main__" and len(sys.argv) == 3 and sys.argv[1] == "--apply-update":
    from aiquota.update_installer import run_update_job
    sys.exit(run_update_job(sys.argv[2]))

from PySide6.QtCore import Qt, QTimer
from PySide6.QtWidgets import QApplication, QMenu

from aiquota.logging_setup import get_logger, setup_logging
from aiquota.settings import load_settings
from aiquota.tray import TrayController
from aiquota.visuals import display_font, register_bundled_fonts
from aiquota.app_icon import load_app_icon
from aiquota.window import QuotaWindow
from aiquota.worker import QuotaWorker
...
from aiquota.update_installer import acknowledge_update
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info("启动 AIQuota")

    QApplication.setHighDpiScaleFactorRoundingPolicy(
        Qt.HighDpiScaleFactorRoundingPolicy.PassThrough
    )
    app = QApplication([sys.argv[0]])
    app.setApplicationName("AIQuota")
    app.setApplicationDisplayName("AIQuota")
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="AIQuota：本机 Codex 额度与用量统计")
