"""Versioned API-equivalent token prices; never a subscription invoice.
...
def _price_signature(rows):
    ...
    return hashlib.sha256(json.dumps(values, sort_keys=True, allow_nan=False).encode("utf-8")).hexdigest()[:16]
...
def _litellm_rows(data, updated_at):
    ...
    for model, item in data.items():
        ...
        thresholds = {0}
        for field in item:
            match = re.search(r"_above_(\d+)k_tokens", field)
            if match:
                thresholds.add(int(match.group(1)) * 1000)
        for tier, suffix in (("default", ""), ("priority", "_priority"), ("flex", "_flex")):
            for threshold in sorted(thresholds):
                context = "_above_%dk_tokens" % (threshold // 1000) if threshold else ""
                row = {key: (_number(item.get(field + context + suffix)) * 1_000_000
                             if _number(item.get(field + context + suffix)) is not None else None)
                       for key, field in _FIELDS.items()}
                row.update(model=model, service_tier=tier, threshold=threshold,
...
def _models_dev_rows(data, updated_at):
    # This fallback supplies standard prices only, never guesses priority multipliers.
    rows = []
    ...
    for model, item in models.items():
        ...
        for tier in tiers if isinstance(tiers, list) else []:
            if not isinstance(tier, dict):
                continue
            rule = tier.get("tier", {})
            if isinstance(rule, dict) and rule.get("type") == "context" and _number(rule.get("size")) is not None:
                candidates.append((int(rule["size"]), tier))
        for threshold, prices in candidates:
            row = {key: prices.get(key) for key in RATE_KEYS}
            row.update(model=model, service_tier="default", threshold=threshold,
...
class PricingCatalog:
    ...
    def _rebuild(self):
        ...
        self._rows = sorted(merged.values(), key=_row_key)
    ...
    def _archive_version(self):
        ...
        if not path.exists():
            _write(path, {"price_version": self.price_version, "created_at": _utcnow().isoformat(),
                          "unit": "USD per million tokens", "rows": self.rows()})
    ...
    def set_override(self, model: str, rates: dict | None):
        ...
        with self._lock:
            if rates is None:
                ...
                row = _validate_row(raw)
                if not row or any(_number(rates.get(key)) is None for key in RATE_KEYS):
                    raise ValueError("四项每百万 Token 单价必须为有限非负数")
                old = self._overrides.get(model, [])
                self._overrides[model] = [r for r in old if _row_key(r) != _row_key(row)] + [row]
    ...
    def _price(self, record):
        ...
        if rates is None:
            return dict(result, reason="缺少该服务档位与上下文长度组合的明确价格")
        ...
        return dict(result, usd=usd, pricing_status="estimated" if estimated else "priced", rates=dict(rates),
                    reason="；".join(reasons) if reasons else "按最新 API 单价计算；不是订阅实际扣款")

    def _fetch(self, url, etag=None):
        headers = {"Accept": "application/json", "User-Agent": "AIQuota/2 pricing"}
        if etag:
            ...
            for url, parser in ((LITELLM_URL, _litellm_rows), (MODELS_DEV_URL, _models_dev_rows)):
                try:
                    ...
                    if unchanged:
                        ...
                        # Fallback standard rows cannot overwrite cached priority data.
                        retained = [r for r in self._cache.get("rows", [])
                                    if url == MODELS_DEV_URL and r.get("service_tier") != "default"]
                        rows = retained + rows
                    if etag:
                        etags[url] = etag
                    verification_rows = []
