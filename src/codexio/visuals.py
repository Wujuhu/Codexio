...
from codexio.rate_limits import QuotaState, WindowView, format_reset_time
...
class MinimalStyle(QuotaStyleWidget):
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self._five_meta.setWordWrap(True)
        self._week_meta.setWordWrap(True)
    ...
    def _apply_side(
        ...
        meta.setText("%s\n%s" % (title, format_reset_time(view.resets_at)))
...
class _ShadowLabel(QLabel):
    ...
    def _fit_wrapped_height(self):
        if self.wordWrap():
            height = max(self.fontMetrics().height(), self.heightForWidth(max(1, self.width())))
            if height != self.minimumHeight():
                self.setMinimumHeight(height)

    def setText(self, text):
        super().setText(text)
        self._fit_wrapped_height()
...
class _RingCard(QWidget):
    ...
    def paintEvent(self, event) -> None:  # type: ignore[override]
        ...
        box = self.rect()
        small = display_font(min(16, max(12, int(box.height() * 0.10))))
        ...
        caption_height = QFontMetrics(small).boundingRect(QRect(0, 0, box.width(), 1000), text_flags, caption).height()
        ring = max(40, min(box.width(), int(box.height() * 0.72), box.height() - caption_height - 6))
        ring_rect = QRectF(
        ...
        meta_top = math.ceil(ring_rect.bottom()) + 2
        meta = QRect(box.left(), meta_top, box.width(), max(0, box.height() - meta_top))
        painter.setPen(QColor(230, 234, 240, 220))
        painter.setFont(small)
        painter.drawText(meta, text_flags, caption)
        painter.end()
...
class _TileCard(QWidget):
    ...
    def paintEvent(self, event) -> None:  # type: ignore[override]
        ...
        shown = self._display if self._display is not None else percent
        color = QColor(percent_color(percent))
        small = display_font(min(14, max(12, int(rect.height() * 0.13))))
        reset = format_reset_time(self._view.resets_at) if percent is not None else "N/A"
        caption = "%s\n%s" % (self._title, reset)
        text_flags = Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignTop | Qt.TextFlag.TextWordWrap
        caption_height = QFontMetrics(small).boundingRect(QRect(0, 0, rect.width() - 16, 1000), text_flags, caption).height()
        meta = QRect(rect.left() + 8, rect.bottom() - caption_height - 8, rect.width() - 16, caption_height)
        value_rect = QRect(rect.left() + 8, rect.top() + 6, rect.width() - 16, max(1, meta.top() - rect.top() - 10))
        painter.setPen(color)
        font = display_font(max(20, min(int(rect.height() * 0.40), int(value_rect.height() * .85))), bold=True)
        painter.setFont(font)
