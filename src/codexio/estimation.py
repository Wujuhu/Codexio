    ...
    for group in groups:
        for segment, termination in _drop_segments(group["observations"]):
            ...
            row = {"reset_at": int(group["reset"].timestamp()), "plan_type": group["plan_type"],
                   ...
                   "status": "insufficient", "reason": "等待至少 2 个百分点的额度变化",
            ...
            if group["account"] is None:
                ...
                row.update(status="ready", estimated_total_usd=estimate,
                           ...
                           reason="按同期 API 基础价 × Codex 倍率后的费用 / 额度变化推算；仅代表已配置来源及该模型组合")
                if estimated_prices:
                    row.update(status="estimated_prices", reason="部分调用使用参考单价；按已记录费用与额度变化折算，作为参考估值")
