...
from codexio.updates import EXE_NAME, Release, UpdateCancelled, UpdateError, file_sha256, version_tuple
...
def launch_installer(directory: Path, release: Release, cancel: threading.Event, *,
    ...
    if target.name.lower() != EXE_NAME.lower() or target.is_symlink():
        raise UpdateError("请以 Codexio.exe 文件名运行应用后更新")
    helper = directory / "CodexioUpdater.exe"
...
def _install_lock(target: Path):
    ...
    import hashlib
    name = "Local\\Codexio.Update." + hashlib.sha256(str(target).lower().encode("utf-8")).hexdigest()[:24]
    ...
    try:
        ...
        if not acquired:
            raise UpdateError("另一个 Codexio 实例正在更新，请稍后重试")
        yield
    finally:
        if acquired:
...
def acknowledge_update() -> str:
    raw = os.environ.pop("CODEXIO_UPDATE_JOB", "")
    if not raw:
        return ""
    try:
...
def cleanup_updates() -> None:
    ...
    for path in jobs:
        ...
        for name in ("package.bin", "package.part", "CodexioUpdater.exe", "previous.bin"):
