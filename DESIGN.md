...
- **Pricing table:** keep one six-column table with centered headers and cells. Group each model as API Base first, then the applicable Standard / long-context / Fast conversion rows; do not split it into two titled sections. Show conditions on two readable lines, using 50 px rows and 4 px / 8 px cell padding. Keep enough precision for small unit prices, with at least two decimal places. Selecting any row identifies its model and condition; editing always opens that model's Standard API base, and filtering cannot redirect an edit to another model. API base values and derived values come from the same backend catalog; the UI never recalculates modifiers. Cache creation in converted rows uses ordinary input rates without a write premium. Missing base values remain unknown rather than zero. All six columns share available width, with minimum widths and horizontal scrolling when necessary.
