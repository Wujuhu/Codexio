...
`tests/test_upstream.py` 使用临时配置、模拟上游与隔离的代理进程，验证流式字节转发、认证头透传、无重定向、观察上限、恢复冲突、异常退出、更新接续和客户端识别。`tests/test_upstream_ui.py` 验证两端设置、提示取消、退出偏好、自动重启调用和模型展示；`tests/test_usage_queries.py` 验证两种日志关联、费用与筛选不变。
