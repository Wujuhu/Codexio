...
界面通过响应 ID 将记录关联到原始调用，按用户请求展示时聚合其成员。箭头从请求模型右侧向上、再向左折，指向上游模型；型号相同保持紫色，实际响应与对应调用的请求型号不一致时，箭头和上游模型文字显示绿色。聚合行按每次调用分别比较，只要已检测的调用中存在不一致就显示绿色。没有关联就不显示检测标签。子代理只有经过同一路由、且有可关联响应 ID 才会显示。HTTP/SSE 会承接原先可用 WebSocket 的调用；不宣称获取未在官方响应中披露的真正内部模型。
